/*Ex 4-8, K&R 2nd, :___:  2016-01-29 Fri 04:26 PM*/
/*C program to implement a reverse polish calculator*/
/*Extensions in this Excercise:
  1. Modify getch() and ungetch() keeping in mind that there
     will never be more than one character of pushback
*/
...
/*Modified for Ex 4-8 - Now it looks for only one 
  character in buffer
*/
int getch(void)
{
...
/*Modified for Ex 4-8 - Now it pushes back only one 
  character onto the buffer
*/
void ungetch(int c)
{
	if(bufp > 0)
...
void ungets(const char s[])
{
	...
	*/
	...
	/*Call ungetch() for these reasons:
	  1. Reuse of code
	  2. ungetch() can be modified in future
	     without impacting ungets()
	  3. In a crude way we can see how many 
	     character are in excess
	*/
	i = strlen(s);
	--i;
	while(i>=0)
