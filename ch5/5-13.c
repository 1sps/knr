/* K&R2 5-13 */
/* ___: 2016-02-17 Wed 09:35 PM */
/* Note: No bad input cases are dealt with in this version */
...
void writelines(char *lineptr[], int nlines, int nprint);
int getnum(char arg[]);
...
int main(int argc, char *argv[])
{
	...
	if(argc > 1)
		nprint = getnum(argv[1]);
	else
		nprint = 10;

	if ((nlines = readlines(lineptr, linebuf, MAXLINES)) > 0)
	{
		writelines(lineptr, nlines, nprint);
		return 0;
	}
	else
	{
...
void writelines(char *lineptr[], int nlines, int nprint)
{
	...
	/* for (i = 0; i < nlines; i++) */
	for (i = nlines - nprint; i < nlines; i++)
		printf("%s\n", lineptr[i]);
}
