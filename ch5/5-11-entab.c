	...
	while((c = getchar()) != EOF)
	{
		...
		else
		{
			...
			if(prev_blank == TRUE)
			{
				...
				virtual_tot_blanks = num_of_blanks + (pos_blanks_start % ts);
				num_of_tabs_needed = virtual_tot_blanks / ts;
				num_of_blanks_needed = virtual_tot_blanks % ts;
			...
			/*Increment porper count for tab*/
				...
				pos_count = pos_count + ts - (pos_count % ts);
